...
export default function IconWordSegmentedToggle({
  ...
  const safeHover =
    hoverPreviewIndex !== null && hoverPreviewIndex < n ? hoverPreviewIndex : null;
  const pillIdx = safeHover ?? idx;
  ...
  return (
    <div
      ...
      onPointerLeave={() => setHoverPreviewIndex(null)}
    >
      <div
        className="pointer-events-none absolute top-1 bottom-1 rounded-lg bg-primary-white shadow-[0_1px_2px_rgb(0_0_0/0.06),0_2px_6px_rgb(0_0_0/0.04)] ring-1 ring-black/[0.06] transition-[left,width,transform,box-shadow] duration-300 ease-[cubic-bezier(0.25,0.9,0.32,1.02)] group-hover:-translate-y-px group-hover:shadow-[0_2px_4px_rgb(0_0_0/0.07),0_4px_12px_rgb(0_0_0/0.06)] [.group:has(button:active)_&]:translate-y-px [.group:has(button:active)_&]:shadow-[0_1px_1px_rgb(0_0_0/0.05),inset_0_1px_2px_rgb(0_0_0/0.06)] dark:bg-zinc-700 dark:ring-white/[0.08] dark:group-hover:shadow-[0_2px_6px_rgb(0_0_0/0.35),0_6px_14px_rgb(0_0_0/0.25)] motion-reduce:transition-[left,width] motion-reduce:duration-200 motion-reduce:group-hover:translate-y-0 motion-reduce:group-hover:shadow-sm motion-reduce:[.group:has(button:active)_&]:translate-y-0 motion-reduce:[.group:has(button:active)_&]:shadow-sm"
        style={{
          width: `calc((100% - 0.5rem) / ${n})`,
          left: `calc(0.25rem + (100% - 0.5rem) * ${pillIdx} / ${n})`,
        }}
        aria-hidden
      />
      <div
        ...
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              onPointerEnter={() => setHoverPreviewIndex(i)}
              className={`flex min-w-0 origin-center items-center justify-center rounded-lg text-xs font-medium [-webkit-tap-highlight-color:transparent] transition-[transform,color,opacity] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 active:scale-[0.86] active:translate-y-[3px] active:duration-75 active:ease-[cubic-bezier(0.33,0,0.67,1)] motion-reduce:transition-colors motion-reduce:active:transform-none motion-reduce:transform-none dark:focus-visible:ring-gray-500 ${
                showLabels
