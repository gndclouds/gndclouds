...
export async function getAllMarkdownFiles(): Promise<Study[]> {
  try {
    // Get content from the studies directory
    const studyPaths = await getMarkdownFilePaths("studies");
    console.log(`Found ${studyPaths.length} study markdown files`);

    const files = await Promise.all(
      studyPaths.map(async (relativePath) => {
        try {
          ...
          console.error(`Error processing study ${relativePath}:`, error);
          return null;
        }
      })
    );
    ...
    const validFiles = files.filter(Boolean) as Study[];
...
export async function getStudyBySlug(slug: string): Promise<Study | null> {
  const allStudies = await getAllMarkdownFiles();
